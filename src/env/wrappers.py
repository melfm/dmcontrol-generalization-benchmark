...
from robosuite.wrappers import GymImageDomainRandomizationWrapper
...
def make_robosuite_env(task_name,
    ...
    env_kwargs['controller_configs'] = load_controller_config(
    ...
    env = GymImageDomainRandomizationWrapper(
        env,
...
class ColorWrapper(gym.Wrapper):
    ...
    def _get_dmc_wrapper(self):
        ...
        assert isinstance(
            _env,
            dmc2gym.wrappers.DMCWrapper), 'environment is not dmc2gym-wrapped'
...
def do_green_screen(x, bg):
    ...
    for x in range(x_w):
        for y in range(x_h):
            r, g, b, a = pix[x, y]
            h_ratio, s_ratio, v_ratio = rgb_to_hsv(r / 255., g / 255.,
                                                   b / 255.)
...
class VideoWrapper(gym.Wrapper):
    ...
    def _load_video(self, video):
        ...
        assert cap.get(cv2.CAP_PROP_FRAME_WIDTH
                       ) >= 100, 'width must be at least 100 pixels'
        assert cap.get(cv2.CAP_PROP_FRAME_HEIGHT
                       ) >= 100, 'height must be at least 100 pixels'
        n = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
        buf = np.empty((n, int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)),
    ...
    def _greenscreen(self, obs):
        ...
        if 'video' in self._mode:
            ...
            bg = self._interpolate_bg(
                bg, obs.shape[1:])  # scale bg to observation size
